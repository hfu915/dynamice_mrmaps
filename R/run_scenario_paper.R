...
load(file = "inputs/dat_income.rds")           # input country and income data
...
var <- list (
  ...
  countries                = dat_income [income_g != "High income", country_code]  # countries for evaluation
)
...
for (index in 1:length(vaccine_strategies)){
  ...
  output_burden_estimate (save_scenario          = sprintf ("scenario%02d", index),
                          foldername             = paste0 ("20221117", "_v2_s1_deter"),
                          sim_years              = 1980:2040,
                          cfr_option             = "Portnoy",
